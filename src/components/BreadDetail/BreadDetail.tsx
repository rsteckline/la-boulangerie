...
const BreadDetail = () => {
  const { id: countryName, breadId: breadName } = useParams<{ id: string; breadId: string }>();
  const [breadDetail, setBreadDetail] = useState<BreadAttributes | null>(null);

  useEffect(() => {
    if (countryName && breadName) {
      console.log(`Fetching bread detail for ${breadName} from ${countryName}...`);
      fetchBreadsForCountry(countryName) 
        .then((data) => {
          console.log('Bread detail data:', data);
          const bread = data.breads.data.find((bread) => bread.attributes.name === breadName);
          if (bread) {
